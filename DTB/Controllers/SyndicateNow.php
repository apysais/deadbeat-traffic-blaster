...
class DTB_Controllers_SyndicateNow extends DTB_Base{
	...
	public function post_syndicate_now(){
		...
		//get posts
		$exclude_this = array();
		if( isset($_POST['exclude_this']) 
			&& !empty($_POST['exclude_this'])
		){
			$exclude_this = $_POST['exclude_this'];
		}
		...
		if( isset($_POST['posts']) ){
					...
					foreach($syndicate as $key_syndicate => $val_syndicate){
						if( isset($val_syndicate['service']) ){
							...
							switch($val_syndicate['service']){
								case 'facebook':
									if( !in_array('facebook', $exclude_this) ){
								...
								break;
								case 'twitter':
									if( !in_array('twitter', $exclude_this) ){
										$cred = array(
								...
								break;
								case 'wordpress':
									if( !in_array('wordpress', $exclude_this) ){
										$content = array(
								...
								break;
								case 'tumblr':
									if( !in_array('tumblr', $exclude_this) ){
			...
			DTB_Notice::get_instance()->set($result_syndicate);
			$this->dbtb_syndicate_now();
			//print_r(DTB_Notice::get_instance()->get());
			//exit();
			//dbtb_redirect(DTB_Admin_SyndicateNow::get_instance()->redirect_url());
		}
