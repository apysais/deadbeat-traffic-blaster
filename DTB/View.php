...
class DTB_View{
	 ...
	 * */
	public function admin_partials($template_file, $data = array()){
	 ...
	 * @return require | bool, if the file exists return the directory path, else false
	 * */
	public function public_partials($template_file, $data = array()){
		/**
		 * path to admin template
		 * */
		$template_admin = $this->public_part_partials($template_file);
		/**
		 * extract the data so it will be variable only
		 * sample:
		 * $data['one'] = 1;
