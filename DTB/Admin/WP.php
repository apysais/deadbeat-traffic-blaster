...
class DTB_Admin_WP {
	...
	public function add_admin_submenu() {
		...
		add_submenu_page(
			$parent,
			esc_html__( 'WP', $textdomain ),
			esc_html__( 'Create New WordPress', $textdomain ),
			'manage_options',
			$this->menu_slug(),
			array(DTB_Controllers_WP::get_instance(), 'controller')
		);
	}
	
	public function get_credentials($account_id){
		$cred = DTB_Admin_AccountDB::get_instance()->get_by_service_id($account_id, 'wordpress');
