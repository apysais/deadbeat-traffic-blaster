...
class DTB_Admin_Facebook {
	...
	public function get_pages_id($id = null){
		$pages = array();
				
		$fb = DTB_Admin_AccountDB::get_instance()->get_by_service_id($id, 'facebook');
		if($fb){
			$cred = DTB_Admin_Facebook::get_instance()->get_credentials($fb->account_id);
			$me = DTB_API_Facebook::get_instance()->me_account(
				$cred['app_id'], 
				$cred['app_secret'], 
				$cred['fb_access_token']
